...
# Initialize Persistent Session State for Evaluation & AI
...
def generate_pdf_report(equipment_name, param_data, notes, ai_summary, logo_bytes, photo_bytes, include_as_found, meta):
    ...
    meta_data = [
        [
            Paragraph(f"<b>Report Made For:</b> {meta.get('made_for', '')}", styles['Normal']),
            Paragraph(f"<b>Equipment / Tag #:</b> {meta.get('equip_num', '')}", styles['Normal'])
        ],
        [
            Paragraph(f"<b>Report Made By:</b> {meta.get('made_by', '')}", styles['Normal']),
            Paragraph(f"<b>Date:</b> {meta.get('date', '')}", styles['Normal'])
        ]
    ]
...
with tab_report:
    ...
    if submitted:
        ...
        st.session_state.last_results = input_results
    ...
    if st.session_state.evaluated:
        ...
        # --- AI DIAGNOSTICS & PERSISTENT EDITING SECTION ---
        st.markdown("---")
            ...
            if active_api_key:
                try:
                    st.session_state.ai_text_val = analyze_with_ai(
                        selected_equipment, st.session_state.last_results, st.session_state.last_notes, active_api_key
                    )
                except Exception as e:
                    st.session_state.ai_text_val = f"AI analysis failed: {e}"
            else:
                st.session_state.ai_text_val = "API Key missing. Please set GEMMA_API_KEY in Streamlit Secrets or sidebar."
